function ll = RDlike(rRDtrans,kRD,gtrans,l,ACTCOMB,ACTCOMBPm,D1,D2)

rRD = exp(rRDtrans); % transformed r is logged
...
litB = sum(log(CPB+eps).*(repmat(ACTCOMB,[1 1 4])==repmat(permute(1:4,[3 1 2]),[N T 1])),3);
litN = sum(log(CPN+eps).*(repmat(ACTCOMB,[1 1 4])==repmat(permute(1:4,[3 1 2]),[N T 1])),3);



K = repmat(kRD,[1 T]);

ll = sum(K.*litB+(1-K).*litN,2);
